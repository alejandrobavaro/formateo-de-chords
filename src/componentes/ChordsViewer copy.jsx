  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        let songsArray = Array.isArray(data) ? data : (typeof data === 'object' && data !== null) ? [data] : [];
  ...
  useEffect(() => {
    if (searchQuery && songsData.length > 0) {
      const filtered = songsData.filter(song =>
        ...
        (song.contenido?.some(col =>
          col.secciones.some(section =>
            section.lineas.some(linea =>
              linea.letra?.toLowerCase().includes(searchQuery.toLowerCase()) ||
              linea.acordes?.some(acorde =>
                acorde?.toLowerCase().includes(searchQuery.toLowerCase())
              )
            )
          )
        )) ||
        // Para estructura antigua
        (song.Secciones?.some(section =>
          section.letra?.toLowerCase().includes(searchQuery.toLowerCase()) ||
          section.acordes?.some(chord =>
